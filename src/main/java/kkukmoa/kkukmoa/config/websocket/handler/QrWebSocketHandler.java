...
public class QrWebSocketHandler extends TextWebSocketHandler {
  ...
  private static final ConcurrentHashMap<String, WebSocketSession> emailSessionMap = new ConcurrentHashMap<>();
  ...
  public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
    ...
    log.info("[+] afterConnectionClosed - Session : " + session.getId() + " Reason : " + status.getReason());
  }


  @Override
  public void handleTransportError(WebSocketSession session, Throwable exception) {
    log.warn("[+] handleTransportError - session :: " + session.getId() + " Reason : " + exception.getMessage());
  }
  ...
  public void sendMessageToEmail(String email, String message) {
    QrGeneralTextDto messageDto = QrGeneralTextDto.builder()
        .message(message)
        .build();
