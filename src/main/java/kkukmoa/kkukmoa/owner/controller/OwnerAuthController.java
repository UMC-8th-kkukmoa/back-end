...
public class OwnerAuthController {
    ...
    @Operation(
            summary = "사장님 로그인",
            description = "전화번호와 비밀번호로 사장님 계정을 로그인합니다."
    )
    @ApiErrorCodeExamples(
            value = {
                    ErrorStatus.USER_NOT_FOUND,
                    ErrorStatus.PASSWORD_NOT_MATCH
            }
    )
    public ResponseEntity<ApiResponse<TokenResponseDto>> loginOwner(@RequestBody @Valid OwnerSignupRequest request) {
    ...
    @Operation(
            summary = "입점 신청 API",
            description = "로그인한 사장님이 매장을 등록(입점 신청)합니다."
    )
    @ApiErrorCodeExamples(
            value = {
    ...
    public ResponseEntity<ApiResponse<String>> registerStore(
            @RequestBody @Valid OwnerRegisterRequest request,
