...
public class OwnerQueryService {
  ...
  /**
   *
   * 스탬프의 QR 코드를 생성하고 반환합니다.
   * QR 코드 생성 시, QR 코드 정보를
   *
  ...
  public OwnerQrResponseDto.QrDto getStamp() {
    ...
    Store store = storeRepository.findByOwner(currentOwner)
   ...
   *
   * 하나의 Store에 대해 항상 하나의 스탬프 QR 코드만 유지되록 관리합니다.
   * 이미 저장된 QR 코드가 있으면 해당 QR 코드에 대한 Redis 키들은 삭제됩니다.
   * 새로 생성되는 QR 코드에 대한 Redis 소스는 다음과 같습니다
   *    - key:  'qrCode:{storeId}' -> value: {qrSource}
  ...
  private void makeOrUpdateQrCode(String qrSource, String storeId) {
    ...
    redisTemplate.opsForValue()
        .set(qrCodeKey, qrSource, 60, TimeUnit.SECONDS);

    // key: QR, value: storeId
    redisTemplate.opsForValue()
        .set(qrStoreKey, storeId, 60, TimeUnit.SECONDS);
