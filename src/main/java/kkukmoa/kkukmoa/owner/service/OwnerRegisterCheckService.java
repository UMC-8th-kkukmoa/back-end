...
public class OwnerRegisterCheckService {
    ...
    /**
     * [비로그인용]
     * 연락처/비밀번호로 본인 확인 후 PENDING 여부 반환.
     * - 사용자 미존재/비번 불일치는 동일 메시지로 응답하여 계정 유추 방지
     * - 존재 여부는 JPQL existsPending(...)으로 즉시 확인 (인덱스 있으면 매우 빠름)
     */
    public OwnerRegisterCheckResponse checkPending(OwnerSignupRequest req) {
        ...
        boolean hasPending = storeRegistrationRepository
                .existsPending(user.getId(), StoreRegistrationStatus.PENDING);
    ...
    /**
     * [로그인 사용자용]
     * 이미 인증된 사용자(userId) 기준으로 PENDING 여부만 확인.
     * - 비밀번호 검증 없음
     * - 소셜/일반 로그인 공통 사용
     */
