...
public class VoucherController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<VoucherResponseDto.VoucherListResponseDto>>> getMyTickets() {
        List<VoucherResponseDto.VoucherListResponseDto> result = voucherQueryService.getMyVouchers();
    ...
    @GetMapping("/{uuid}")
    public ResponseEntity<ApiResponse<VoucherResponseDto.VoucherDetailResponseDto>> getTicketDetail(@PathVariable String uuid) {
    ...
    @Operation(
        ...
        - QR UUID와 차감 금액을 함께 전송합니다.
        - 남은 금액이 0이 되면 상태가 USED로 변경됩니다.
        """)
    ...
    public ResponseEntity<ApiResponse<String>> useVoucher(
            @Parameter(description = "금액권 QR UUID (예: voucher_3ad5c7c6-xxxx-yyyy)", example = "voucher_3ad5c7c6-3c5a-4b96-a5e7-bf9201795a42")
            @PathVariable String uuid,

            @Parameter(description = "차감할 금액 (원)", example = "5000")
            @RequestParam("amount") int useAmount
    ) {
