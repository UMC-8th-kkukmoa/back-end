...
public class VoucherController {
    ...
    /**
     * 내 금액권 사용내역 조회 (커서 기반 무한스크롤)
     * - 최신 사용내역부터 정렬 (usedAt DESC, id DESC)
     * - 커서: base64url("epochMillis:id") 형식
     */
    @Operation(
    ...
    @GetMapping("/usage")
    public ResponseEntity<ApiResponse<VoucherResponseDto.CursorPageResponse<VoucherUsageRow>>> getMyUsage(
            @Parameter(description = "페이지당 조회 개수 (최대 100)", example = "5")
            @RequestParam(required = false) Integer limit,

            @Parameter(description = "커서 (base64url(\"epochMillis:id\") 형식)", example = "MT725500000000:123")
